...
import ClassroomManager from './chamada_dashboard.jsx'

{/* Componente Auxiliar para os Botões do Menu */}
        const NavButton = ({icon, label, active, onClick}) => (
        <button
          onClick={onClick}
          className={`flex flex-col items-center gap-1 p-2 rounded-xl transition-all ${active ? 'text-blue-400' : 'text-slate-500 hover:text-slate-300'
            }`}
        >
          ...
          {/* Removido o 'hidden' para que o transition-opacity funcione */}
  ...
  return (
    <div className="w-full min-h-screen bg-slate-950 text-white font-sans relative">
      ...
      <main className="p-6 pb-28 animate-fade-in">
        ...
        {activeTab === 'home' && (
          <>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <button onClick={() => setActiveTab('classroom')} className="group bg-slate-900 hover:bg-slate-800 border border-slate-800 hover:border-blue-500/50 p-4 rounded-2xl transition-all duration-300 flex flex-col items-start gap-3 relative overflow-hidden">
        ...
        {/* --- VIEW: GESTÃO DE SALA DE AULA (DIÁRIO) --- */}
        {activeTab === 'classroom' && (
          <ClassroomManager onBack={() => setActiveTab('home')} />
        )}
